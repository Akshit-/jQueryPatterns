...
//REFACTORED avoiding requery by storing the frequent used jquery objects
...
function setupHealthBars() {
    //REFACTORED by using find
  var $youHealthBarContainer = $('#you').find('.health-bar'),
    $opponentHealthBarContainer = $('#opponent').find('.health-bar'),
    ...
    ////REFACTORED by using $.data() and not $(context).data()
    $.data(youHealthBar, 'origHeight', .8*height);//youHealthBar.data('origHeight', .8*height);
    $.data(youHealthBar, 'origWidth', .8*width);//youHealthBar.data('origWidth', .8*width);
    $.data(opponentHealthBar, 'origHeight', .8*height);//opponentHealthBar.data('origHeight', .8*height);
    $.data(opponentHealthBar, 'origWidth', .8*width);//opponentHealthBar.data('origWidth', .8*width);
    $.data($youHealthBarContainer, 'healthBar', youHealthBar);//$youHealthBarContainer.data('healthBar', youHealthBar);
    
    $.data($opponentHealthBarContainer, 'healthBar', opponentHealthBar);//$opponentHealthBarContainer.data('healthBar', opponentHealthBar);
...
function setupKeyHandlers() {
  $(document).keydown(function (e) {
    ...
    sequenceSoFar.push(e.keyCode);
     ...
     //REFACTORED using find and caching it for all cases
    var youpokemonel = $('#you').find('.pokemon')[0];
  ...
  var clientId = 'df1eacadddaf233fdf1c1192a27b7ce5';
    ...
    SC.connect(function() {
      SC.get("/me/playlists", function(playlists) {
        ...
        //REFACTORING by appending to dom outside loop
        var tracksHtml = "";
